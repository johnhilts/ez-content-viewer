...
- =get-file-info= is a =mapcar= predicate.
  ...
  - [ ] compare (truename) values
  - [ ] we can add another slot for linked-path (call it =alias-path=) - if it exists, that's what we'll use instead of file-path
  - [ ] hopefully that'll be enough to get the images to display
** Notes
   #+begin_src lisp
