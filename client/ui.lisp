  ...
  (labels ((get-file-dimensions (height width is-landscape)
    ...
    (flet ((highlight-selected-element ()
           ...
           (search-favorites (event)
             ;; favoriteList.filter(favorite => favorite.name.toLowerCase().indexOf("cal") >= 0).map(favorite => favorite.name)
             ;; (add-favorite-search-result (random 12345))
             ...
             (let ((search-input (chain (+ (@ (chain document (get-element-by-id "favorite-search")) value) (@ event key)) (to-lower-case))))
               (chain (chain favorite-list
                      ...
                      (map #'(lambda (favorite) (add-favorite-search-result (@ favorite name)) t))))
             t))
                   ...
                   (div (class . "column-item")
                        ...
                        (div (id . "overlay")
                             (div (id . "text")
                                  ...
                                  (h2 "Add to Favorites")
                                  (input (id . "favorite-search") (placeholder . "search") (onkeypress . "(search-favorites)"))
                                  (span " " (button (onclick . "(add-favorite-item (@ file path))") "add new") (br " "))
