...
;;  132.0
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; $Id: exifinfo.cl,v 1.3 2002/12/27 00:42:00 layer Exp $
...
(defun get16u (data offset)
  (if (eq :motorola *endian*)
      (logior (ash (aref data offset) 8)
              (aref data (1+ offset)))
    (logior (ash (aref data (1+ offset)) 8)
            (aref data offset))))
...
(defun process-sof (file data res
  ...
  (let ((len (get16m data 3))
	(wid (get16m data 5)))
    (if res
	(progn (setf (exif-info-image-length res) len)
	       (setf (exif-info-image-width res) wid)
	       res)
      (progn (push len init-forms)
...
(defun process-exif-dir (res file data dir-start-offset
  ...
  (let ((n-dir-entries (get16u data dir-start-offset)))
	...
	(if (> byte-count 4)
	    (let ((offset-val (get32u data (+ dir-entry 8))))
...
(defun get-string (data start end)
  (do ((s start (1+ s)))
      ((= s end) (sb-ext:octets-to-string data :start start :end end))
    (when (= 0 (aref data s))
      (return (sb-ext:octets-to-string data :start start :end s)))))
